...
def read_diff(args):
    ...
    if args.staged:
        ...
        # Validate before this reaches git's argv. Unchecked, a --base of
        # "--output=/tmp/x" becomes the token "--output=/tmp/x...HEAD", which
        # git diff honours as an output-file option — an arbitrary file-write
        # primitive. Not shell injection (argv is a list), but argument
        # injection, and an agent builds this value from user prose.
        if not re.fullmatch(r"[A-Za-z0-9._/@^~-]+", args.base):
            raise SystemExit("scope_creep: --base must be a plain revision name")
