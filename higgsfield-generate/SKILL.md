...
## Credits & cost — read before any generation

**Every MCP / CLI generation burns real credits, even on an "Unlimited" plan.** Higgsfield's "Unlimited" / "365 Unlimited" entitlements (GPT Image 365, Seedance 1.5 Pro, Kling 2.6, Nano Banana, etc.) apply ONLY to manual generation in the **web app** (Image/Video/Cinema Studio). The API/MCP/CLI always draws down the credit balance at the nominal rate.

- Never tell the user a model is "free" just because it shows Unlimited on their plan — through this skill it costs credits.
- A cost preflight (`get_cost: true`) returns the nominal price (**not zero**) for unlimited models; it does not reflect the web-app entitlement.
- Watch the balance: `higgsfield account status` before and after a batch.
- Indicative MCP costs: Seedance 1.5 Pro ~2.4 (480p/4s), Kling 2.6 ~10 (5s), Veo 3.1 ~11 (4s), gpt_image_2 ~4 (1k/high) / ~7 (2k/high).
- For genuinely free generation on an unlimited model, the user must generate manually in the web UI — or use the web-API replay method below.

## Free generation under an Unlimited plan (web-API replay)

Replay the web app's own API call from a logged-in browser page (gstack `/connect-chrome` → persistent profile) to get unlimited generation **for free**. The `use_unlim:true` flag is what makes it free.

- Endpoint: `POST https://fnf.higgsfield.ai/jobs/v2/seedance1_5`
- Auth: `Bearer <token>` from `await window.Clerk.session.getToken()` **in the page context** — refresh the token **every clip** (Clerk JWTs expire ~60s; reusing one across a loop → 401s).
- Do NOT send `credentials:'include'` (stricter CORS → "Failed to fetch").
- Body must include `"use_unlim": true` both top-level and inside `params`. Upload start-frames via MCP `media_upload` first (free, no DataDome), then inject the returned id as `medias[].data.id`.

**Hard limits / gotchas:**
- **DataDome anti-bot**: rapid direct fetches trip DataDome → `403` HTML page; does not cool down in minutes. Throttle hard or accept partial + resume later. The real UI "Generate" button keeps working.
- **16-concurrent-job cap** (Team): the 17th in-flight job fails. Submit in waves of ≤14, let them drain (~3 min/render) before the next wave.
- **Browser instability**: headed Chromium drops the Clerk session / goes `about:blank` after ~150s of heavy use. Keep in-page loops < ~120s; reconnect (`browse connect`) on crash — the persistent-profile login is restored.
- **Start-frame swap bug**: re-uploading into an occupied `<input type=file>` silently fails (the slot only accepts an upload when empty) → every clip animates the same image. Fix: pass `medias[].data.id` directly in the body.
- **MCP `generate_video` has NO audio for `seedance_1_5`** (clips are silent). For audio via MCP use Veo 3.1 / Kling / Seedance 2.0 (they cost credits). To force English native audio in the prompt: `AUDIO IN ENGLISH ONLY: a warm female voice says: "<line>". English speech only, no Chinese.`
- To stop the model spinning a flat ad graphic: `the jars are completely static and locked in place: they do NOT spin, do NOT rotate. Only the camera moves slowly forward.`

**Reconcile after firing** (don't trust the in-page log — the page may die): call MCP `show_generations` and match `params.medias[0].data.id` (filter `params.prompt` for your batch's unique phrase) against your target media-id list; resubmit only the missing ones. Download via `results.rawUrl`.
...
5. Don't pre-estimate cost in chat. Just submit unless the user asks — but remember every MCP/CLI run burns real credits even on an "Unlimited" plan (see **Credits & cost** above).
